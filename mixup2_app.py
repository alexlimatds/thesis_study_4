...
import pandas as pd
import numpy as np
import csv, random, torch, singlesc_models, transformers, time, json, malik, facts, mixup2_models
...
def evaluate_BERT(train_params):
    ...
    if train_params.get('n_documents') is not None: # used in tests to speed up the train procedure
        ...
        ds_train = singlesc_models.get_dataset(temp_docs_train, labels_to_idx, tokenizer)
        ds_test = singlesc_models.get_dataset(temp_docs_test, labels_to_idx, tokenizer)
    else:
        ds_train = singlesc_models.get_dataset(dic_docs_train, labels_to_idx, tokenizer)
        ds_test = singlesc_models.get_dataset(dic_docs_test, labels_to_idx, tokenizer)
    ...
    for i, seed_val in enumerate(seeds):
        ...
        # training
        iteration_metrics, cm, train_time, n_augmented_vectors = mixup2_models.fit(train_params, ds_train, ds_test, labels_to_idx, device)
...
def save_report(
    ...
    confusion_matrices, evaluation, train_params, 
    train_time, device, time_tag, n_augmented_vectors):
    """
    Arguments:
    ...
    report = (
        ...
        f'Number of epochs: {train_params["n_epochs"]}\n'
        ...
        f'Average number of mixup vectors by epoch: {n_augmented_vectors}\n'
    ...
    with open(f'./reports/{dataset_name}/rep-mixup2-{time_tag}.txt', 'w') as f:
        f.write(report)
