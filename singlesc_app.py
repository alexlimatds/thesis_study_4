...
import pandas as pd
import numpy as np
import csv, random, torch, singlesc_models, transformers, time, json, malik, facts
...
def evaluate_BERT(train_params):
    ...
    for i, seed_val in enumerate(seeds):
        ...
        # train
        iteration_metrics, cm, train_time = singlesc_models.fit(train_params, ds_train, ds_test, device)
...
def save_report(
    ...
    report = (
        ...
        f'Freeze layers: {train_params["freeze_layers"]}\n'
    ...
    with open(f'./reports/{dataset_name}/rep-singlesc-{time_tag}.txt', 'w') as f:
        f.write(report)
