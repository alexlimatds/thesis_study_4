...
import pandas as pd
import numpy as np
import csv, random, torch, dfcsc_sep_models, dfcsc_cls_models, transformers, time, json, malik, facts
from datetime import datetime

def evaluate_model(train_params):
    # time tag
    model_reference = train_params['model_reference']
    time_tag = f'DFCSC-SEP_{model_reference}_{datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss")}'
    ...
    for i, seed_val in enumerate(seeds):
        ...
        # train
        iteration_metrics, cm, train_time = dfcsc_sep_models.fit(train_params, ds_train, ds_test, device)
